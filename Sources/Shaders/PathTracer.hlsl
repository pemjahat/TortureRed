...
void CSMain(uint3 dispatchThreadID : SV_DispatchThreadID)
{
    ...
    RNG rng;
    rng.state = pcg_hash(launchIndex.y * launchDims.x + launchIndex.x + g_Frame.frameIndex * launchDims.x * launchDims.y);
    
    float2 d = (((float2)launchIndex + float2(next_float(rng), next_float(rng))) / (float2)launchDims) * 2.0f - 1.0f;
    d.y = -d.y;
