// RestirGI_Specular_Temporal.hlsl — RTR stream
// VNDF/GGX-sampled specular reflections with short temporal history.
// Reuses diffuse candidate when roughness >= rtrRoughReuseThreshold.
...
static const float RESTIR_TEMPORAL_MAX_HISTORY_LENGTH = 3.0f;
static const uint  RESTIR_TEMPORAL_MAX_HISTORY_AGE = 12u;
static const float RESTIR_TEMPORAL_MAX_JACOBIAN = 1.5f;
static const float RESTIR_TEMPORAL_MIN_JACOBIAN = 0.1f;
static const float RESTIR_TEMPORAL_INIT_GAIN_CLAMP = 3.0f;
static const float RESTIR_TEMPORAL_REUSE_WEIGHT_CLAMP = 8.0f;
static const float RESTIR_TEMPORAL_REFLECTION_THRESHOLD = 0.995f;

// Force specular-only sampling: GGX VNDF importance sampling
void SampleSpecularRay(float3 N, float3 V, float3 baseColor, float metallic, float roughness,
                       inout RNG rng, out float3 rayDir, out float3 throughput, out float pdf)
{
    ...
    throughput = (F_spec * G * VdotH) / (NdotV * NdotH + 0.0001f);
    pdf = (D * NdotH) / (4.0f * VdotH + 0.0001f);
}

// Specular-only target PDF
...
float GetSpecularTargetShape(Surface s, float3 samplePos)
{
    float3 L = normalize(samplePos - s.worldPos);
    float dotNL = max(0.0f, dot(s.normal, L));
    ...
    float3 d, spec;
    EvaluateBSDF(s.normal, s.viewDir, L, s.albedo, s.metallic, s.roughness, d, spec);
    return max(0.0f, Luminance(spec * dotNL));
}
...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    RNG rng;
    seed_rng(rng, screenPos, g_Frame.frameIndex + 7u); // Offset seed from diffuse

    // InputIdx0 = previous specular reservoirs (SRV)
    // InputIdx1 = diffuse candidate buffer (SRV) — for rough-surface reuse
    // OutputIdx0 = current specular reservoirs (UAV)
    // OutputIdx1 = debug heatmap (UAV, optional)
    StructuredBuffer<Reservoir>          prevReservoirs = ResourceDescriptorHeap[g_Indices.InputIdx0];
    StructuredBuffer<DiffuseCandidate>   diffuseCandidates = ResourceDescriptorHeap[g_Indices.InputIdx1];
    ...
    // Determine whether to reuse diffuse candidate (Kajiya rough-surface strategy)
    bool reuseDiffuseCandidate = (surface.roughness >= g_Frame.rtrRoughReuseThreshold);
    ...
    if (reuseDiffuseCandidate)
    {
        // Read diffuse candidate from RTDGI pass
        DiffuseCandidate dc = diffuseCandidates[pixelIndex];
        if (dc.hitT > 0.0f) {
            hitPos = dc.hitPos;
            hitNormal = dc.hitNormal;
            continuationRadiance = dc.radiance;
            firstBounceHitT = dc.hitT;
    ...
    {
        // Trace specular ray (VNDF sampling)
        float3 rayDir;
        float3 throughput;
        SampleSpecularRay(surface.normal, surface.viewDir, surface.albedo, surface.metallic, surface.roughness, rng, rayDir, throughput, pdf);
        ...
        if (isValidSample)
        {
            ...
            if (q.CommittedStatus() == COMMITTED_TRIANGLE_HIT)
            {
                ...
                bool useCachedRadiance = false;
                {
                    ...
                    useCachedRadiance = IsSharcQueryValid(hitPos, q.CommittedRayT(), surface.roughness, sharcParams)
    ...
    {
        float targetPDF = GetSpecularTargetPDF(s, hitPos, continuationRadiance);
        float targetShape = GetSpecularTargetShape(s, hitPos);
        ...
        if (updateReservoir(r, hitPos, hitNormal, continuationRadiance, firstBounceHitT, risWeight, next_float(rng))) {
            ...
            r.historyAge = ReservoirPackAge(0u, true); // Always specular
        }
    }

    // 3. Temporal reuse (specular-optimized: reflection-direction check, short history)
    if (g_Frame.frameIndex > 0u) {
        ...
        if (prevUV.x >= 0.0f && prevUV.x < 1.0f && prevUV.y >= 0.0f && prevUV.y < 1.0f) {
            ...
            if (prevR.M > 0.0f && ReservoirAge(prevR) < RESTIR_TEMPORAL_MAX_HISTORY_AGE) {
                ...
                // Reflection-direction validity check (specular-only guard)
                float3 prevReflectionDir = GetSurfaceReflectionDir(prevSurface);
                bool reflectionMatch = dot(currentReflectionDir, prevReflectionDir) > RESTIR_TEMPORAL_REFLECTION_THRESHOLD;

                if (depthMatch && normalMatch && materialMatch && reflectionMatch) {
                    float historyTargetPDF = GetSpecularTargetPDF(s, prevR.hitPos, prevR.radiance);
                    ...
                    if (historyTargetPDF > 0.0f && jacobianValid) {
                        ...
                        adjustedPrev.historyAge = ReservoirPackAge(
                            min(ReservoirAge(prevR) + 1u, RESTIR_TEMPORAL_MAX_HISTORY_AGE), true);
