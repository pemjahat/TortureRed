// RestirGI_Specular_Spatial.hlsl — Specular spatial reuse pass
// Reads from specular reservoir stream only, uses specular-optimized thresholds.
// Applies reflection-direction validity check per neighbor.
...
static const float RESTIR_SPATIAL_REFLECTION_THRESHOLD = 0.98f;
...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    RNG rng;
    seed_rng(rng, screenPos, g_Frame.frameIndex + 2);
    ...
    if (r.M > 0.f) {
        selectedPDF = GetSpecularTargetPDF(s, r.hitPos, r.radiance);
    }

    // Spatial reuse — specular-optimized: tight Jacobian, reflection-direction check
    int numNeighbors = 3;
    float radius = 20.f;
    ...
    for (int i = 0; i < numNeighbors; ++i) {
        ...
        if (neighborPos.x >= 0 && neighborPos.x < (int)launchDims.x && neighborPos.y >= 0 && neighborPos.y < (int)launchDims.y) {
            ...
            if (hasNeighborHit && neighborR.M > 0.0f && normalsMatch && depthMatch && materialMatch) {
                // Reflection-direction validity check
                float3 candidateDir = normalize(neighborR.hitPos - centerSurface.worldPos);
                if (dot(centerReflectionDir, candidateDir) <= RESTIR_SPATIAL_REFLECTION_THRESHOLD) {
                    continue;
                }

                float neighborTargetPDF = GetSpecularTargetPDF(s, neighborR.hitPos, neighborR.radiance);
