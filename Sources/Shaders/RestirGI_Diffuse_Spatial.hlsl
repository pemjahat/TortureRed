// RestirGI_Diffuse_Spatial.hlsl — Diffuse spatial reuse pass
// Reads from diffuse reservoir stream only, uses diffuse-optimized thresholds.
...
static const float RESTIR_SPATIAL_REUSE_WEIGHT_CLAMP = 64.0f;

// Diffuse-only target PDF
...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    RNG rng;
    seed_rng(rng, screenPos, g_Frame.frameIndex + 1);
    ...
    if (r.M > 0.f) {
        selectedPDF = GetDiffuseTargetPDF(s, r.hitPos, r.radiance);
    }

    // Spatial reuse — diffuse-optimized: no reflection-direction check
    int numNeighbors = 3;
    float radius = 20.f;
    ...
    for (int i = 0; i < numNeighbors; ++i) {
        ...
        if (neighborPos.x >= 0 && neighborPos.x < (int)launchDims.x && neighborPos.y >= 0 && neighborPos.y < (int)launchDims.y) {
            ...
            if (hasNeighborHit && neighborR.M > 0.0f && normalsMatch && depthMatch && materialMatch) {
                float neighborTargetPDF = GetDiffuseTargetPDF(s, neighborR.hitPos, neighborR.radiance);
